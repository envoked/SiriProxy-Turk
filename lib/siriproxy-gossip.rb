...
class SiriProxy::Plugin::Gossip < SiriProxy::Plugin
  ...
  listen_for /Are you listening/i do
    say "Yes. Now let's get down to business" #say something to the user!
  ...
  listen_for /proxy ([^ ]+) (.*)/i do |command,param|
    if param.nil?
      say "I think I need more data."
    else
      say  Net::HTTP.get(URI("http://localhost:8080/delegate/#{command}/#{URI.escape(param)}")) rescue "Something went Wrong"
    end
    request_completed
  end
